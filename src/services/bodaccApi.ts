...
export class BodaccApiService {
  ...
  private static async getDepartmentCreations(dateFrom: string, dateTo: string, signal: AbortSignal): Promise<Record<string, number>> {
    ...
    try {
      ...
      const aggregatesUrl = `${BODACC_DATASET_BASE}/aggregates?${params.toString()}`;
      ...
      if (aggregatesResponse.ok) {
        ...
        console.log('📊 Données aggregates:', aggregatesData);
        
        const departmentData: Record<string, number> = {};
        
        if (aggregatesData.results && Array.isArray(aggregatesData.results)) {
          console.log('🏛️ Structure results trouvée:', aggregatesData.results.length, 'résultats');
          aggregatesData.results.forEach((result: any) => {
          ...
          if (Object.keys(departmentData).length > 0) {
            console.log('✅ Aggregates réussies:', Object.keys(departmentData).length, 'départements');
            return departmentData;
          }
        }
      }
      
      console.log('❌ Endpoint aggregates échoué ou vide');
    } catch (error) {
      console.log('❌ Endpoint aggregates échoué:', error);
    }
    
    // Méthode 3: Échantillonnage avec extrapolation
    console.log('🔄 Tentative méthode échantillonnage...');
    try {
      const whereClause = `dateparution >= date'${dateFrom}' AND dateparution <= date'${dateTo}'`;
      const params = new URLSearchParams();
      params.set('limit', '1000'); // Échantillon plus large
      params.set('where', whereClause);
      
      const sampleUrl = `${BODACC_API_BASE}?${params.toString()}`;
      console.log('🌐 URL échantillonnage:', sampleUrl);
      ...
      if (sampleResponse.ok) {
        ...
        console.log('📊 Données échantillon:', {
          total_count: sampleData.total_count,
          records_count: sampleData.records?.length || 0
        });
        
        const departmentData: Record<string, number> = {};
        
        if (sampleData.records && Array.isArray(sampleData.records)) {
          // Compter par département dans l'échantillon
          ...
          console.log('📊 Départements dans échantillon:', Object.keys(departmentData).length);
          
          // Extrapoler si nécessaire
          if (sampleData.total_count > sampleData.records.length) {
            const ratio = sampleData.total_count / sampleData.records.length;
            console.log(`📊 Extrapolation avec ratio ${ratio.toFixed(2)}`);
            Object.keys(departmentData).forEach(dept => {
              const extrapolated = Math.round(departmentData[dept] * ratio);
              console.log(`📍 Département ${dept}: ${departmentData[dept]} → ${extrapolated} (extrapolé)`);
              departmentData[dept] = extrapolated;
            });
          }
          
          if (Object.keys(departmentData).length > 0) {
            console.log('✅ Échantillonnage réussi:', Object.keys(departmentData).length, 'départements');
            return departmentData;
          }
        }
      }
      
      console.log('❌ Méthode échantillonnage échouée');
    } catch (error) {
      console.log('❌ Erreur échantillonnage:', error);
    }
    
    // Méthode 4: Données simulées en dernier recours
  ...
  private static async getDepartmentCreationsAlternative(dateFrom: string, dateTo: string, signal: AbortSignal): Promise<Record<string, number>> {
    ...
    // Cette méthode est maintenant intégrée dans getDepartmentCreations
    return this.getDepartmentCreations(dateFrom, dateTo, signal);
  }
